<?php
new Conferencer_Shortcode_Agenda_Custom_XML2();
class Conferencer_Shortcode_Agenda_Custom_XML2 extends Conferencer_Shortcode {
	var $shortcode = 'agendacustomxml2';
	var $defaults = array(
		'row_type' => 'track',
...
<?php
          ...
          <?php	$xml[$rowidx]['rooms']['no_name'] = Array('<Cell aid:table="cell" aid:crows="1" aid:ccols="1" aid:ccolwidth="227.97244094547244"><Session>'.htmlspecialchars(get_the_title($session->ID), ENT_XML1).'</Session></Cell>'); ?>
            ...
            <?php if (!empty($do_rows)) {?>
                ...
                <?php $xml[$rowidx]['rooms'][$row_header['title']]['session'][] = $this->get_session($do_row); ?>
                <?php $xml[$rowidx]['rooms'][$row_header['title']]['chair'][] = $do_row->chair; ?>
                <?php $xml[$rowidx]['rows']++ ?>
                <?php } ?> 
           		 <?php } // end $do_row?>
            <?php } ?>
...
<?php }
	...
	function get_session($session , $is_keynote = false) {
		...
		if (!$is_keynote) {
			$session_meta = trim(strip_tags(do_shortcode("[session_meta
							post_id='$session->ID' 
							show='speakers' 
			...
			return $html;?>
		<?php } else { //is keynote ?>
        	<Session><?php echo htmlspecialchars(get_the_title($session->ID), ENT_XHTML);?></Session>
        <?php } ?>
		...
		foreach ($xml as $slot){
			...
			foreach ($slot['rooms'] as $room_name => $room){
				if ($room_name == 'no_name'){
					$out .= '<Cell aid:table="cell" aid:crows="1" aid:ccols="1" aid:ccolwidth="28.542"> </Cell>';
					$out .= $room[0];
				} else {
					$out .= '<Cell aid:table="cell" aid:crows="'.count($room['session']).'" aid:ccols="1" aid:ccolwidth="28.542"><Room>'.$room_name.'</Room>
<Chair>'.get_the_title($room['chair'][0]).'</Chair></Cell>';
					//$out .= '<Cell aid:table="cell" aid:crows="1" aid:ccols="1" aid:ccolwidth="15.307086612992066"><Room>'.$room_name.'</Room></Cell>';
					$out .= implode('', $room['session']);
				}
		...
		if (!$is_keynote) {
			?><SessionType_<?php if ($session->track) echo(get_the_title($session->track));?>  aid:table="cell" aid:crows="1" aid:ccols="1" aid:ccolwidth="227.97244094547244"><Session><SessionTitle><?php echo htmlspecialchars(get_the_title($session->ID), ENT_XML1);?></SessionTitle><?php echo htmlspecialchars(trim(strip_tags (do_shortcode("
						[session_meta
							...
							link_speakers=".($link_speakers ? 'true' : 'false')." 
							link_room=".($link_rooms ? 'true' : 'false')." 
						]"))), ENT_XHTML,'ISO-8859-1');	?></Session></SessionType_><?php if ($session->track) echo(get_the_title($session->track));?>>
		<?php } else { //is keynote ?>
        	<Session><?php echo htmlspecialchars(get_the_title($session->ID), ENT_XHTML, 'ISO-8859-1');?></Session>
        <?php } ?>
